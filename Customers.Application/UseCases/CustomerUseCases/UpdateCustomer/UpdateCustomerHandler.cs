    ...
    public class UpdateCustomerHandler : IRequestHandler<UpdateCustomerRequest, Guid>
    {
        ...
        public async Task<Guid> Handle(UpdateCustomerRequest request, CancellationToken cancellationToken)
        {
            ...
            customerToSave.Payments.ForEach(item =>
            {
                if (item.Id == null || item.Id == Guid.Empty)
                {
                    ...
                    item.DateCreated = Utilities.DateTimeForBrazil();
                }
                else
                {
                    item.DateUpdated = Utilities.DateTimeForBrazil();
                }
            });
            ...
            customerToSave.DateUpdated = Utilities.DateTimeForBrazil();
            ...
            where R : CustomerActionsResponse
        {
            foreach (var item in request)
            {
                ...
                {
                    LogicFindEntity(customer, (Guid)item.Id).DateDeleted = Utilities.DateTimeForBrazil();
                }
                LogicFindEntity(customer, (Guid)item.Id).DateUpdated = Utilities.DateTimeForBrazil();
            }
            return customer;
        }
    }
